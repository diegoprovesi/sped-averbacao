...
class Ndd extends ToolsCommon
{
    ...
    public function averbaXml($cXml, $cTipo)
    {
        ...
        if (empty($this->cUsuario)) {
            throw new \InvalidArgumentException('O usuário da NDD deve ser passado.');
        }
        if (empty($this->cSenha)) {
            throw new \InvalidArgumentException('A senha da NDD deve ser passada.');
        }
        if (empty($this->cCodigo)) {
            throw new \InvalidArgumentException('O código da NDD deve ser passada.');
        }
        switch ($cTipo) {
            case 'CTE':
        ...
        if ($this->tpAmb == 1) {
            $this->cUrl = 'https://wsaverba-prd.e-datacenter.nddigital.com.br/ATMWebSvr.asmx?wsdl';
            $this->cHost = 'webserver.averba.com.br';
        } else {
            $this->cUrl = 'https://wsaverba-hmg.e-datacenter.nddigital.com.br/ATMWebSvr.asmx?wsdl';
